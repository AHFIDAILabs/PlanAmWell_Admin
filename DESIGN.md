...
| Placeholder headshot `<img>`s (Unsplash-style AI-generated portraits) for admin/user avatars | **Initial-letter avatar circles** (already the pattern in the current app's `RecentUsers`/`PendingDoctors`/table rows) | No real photo data exists for most entities; initials are what the current app already does and what several mockups themselves fall back to |
